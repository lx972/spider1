...
function loadArticle(jsonEntity){
  //方法级渲染
  table.render({
   elem: '#listTable'
   ,url: '/article/listByPramas'
   ,width:1560
   ,height: 480
   ,page:true
   ,even:true
   ...
   ,cols: [[
     ...
     ,{field:'md5Key', title: '唯一标识该章节，md5加密后的title',align:'center', width:160, sort: true}
     ,{field:'title', title: '章节标题',align:'center', width:160, sort: true}
     ,{field:'content', title: '章节内容',align:'center', width:160, sort: true}
  ...
  table.on('tool(list)', function(obj){
    ...
    if(obj.event === 'show'){
      ...
      layer.confirm('真的删除行么', function(index){
        ...
        czArticle("/article/delete",jsonEntity,obj);
        layer.close(index);
      });
      ...
      layer.open({
		...
		anim:4,
        shade:0.6,
  		content:$("#updArticleDiv"),
  		success:function(){
        form.val("updArticleForm", {
    "md5Key": data.md5Key,
    "title": data.title,
    "content": data.content,
  		          ...
  		          //监听提交
			  form.on('submit(submitUpdArticle)', function(FormData){
			    var jsonEntity = FormData.field
			    jsonEntity.id = data.id;
			    czArticle("/article/updT",jsonEntity,obj);
			    layer.closeAll();
			    return false;
			  });
  		}
    ...
    $("#delBatchArticle").click(function(){
	   ...
	   if(data.length<1){
          layer.msg("请选择数据！！！",{"icon":3,anim:6,shade:0.6,"time":2000});
           ...
           layer.confirm("你确定要删除所选中的数据吗？？？", {icon: 7, title: '提示'}, function (index) {
               ...
               delManyArticle("/article/deletes", params);
               flush(currentPage);
               layer.close(index);
           });
       }
  ...
  $("#saveUI").click(function(){
	  layer.open({
	    ...
	    shade:0.6,
		  ...
		  form.on('submit(submitUpdArticle)', function(FormData){
            var jsonEntity = FormData.field
		    czArticle("/article/saveT",jsonEntity);
		    layer.closeAll();
		    return false;
		  });
  		}
  ...
  //普通增删改操作
  function czArticle(url,jsonEntity,obj){
	  $.ajax({
		   	type:"post",
		   	url:url,
		   	data:jsonEntity,
		   	success:function(data){
		   		if(data=="delSuccess"){
	   				...
	   				layer.msg("信息修改成功。。。",{"icon":6,anim:4,shade:0.6,"time":2000});
		   			 obj.update({
                                                                                                                md5Key: jsonEntity.md5Key,
                                                                                         title: jsonEntity.title,
                                                                                         content: jsonEntity.content,
                                           		   	      });
  ...
  //批量删除
  function delManyArticle(url,ids){
	  $.ajax({
			type:"post",
			url:url,
			data:{'ids':ids},
